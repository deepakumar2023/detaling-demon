import SEO from '../../components/SEO';
export default function PPFColorTexture() {
  return (
    <div className="min-h-screen pt-48 px-5 md:px-20 bg-[var(--color-background)]">
      <SEO title="Color & Texture PPF | Detailing Demons" description="Learn more about our Color & Texture PPF services." />
      <h1 className="text-4xl font-bold font-[Space_Mono] text-[var(--color-primary-container)] mb-8">
        Color & Texture PPF
      </h1>
      <p className="text-[var(--color-on-surface-variant)] font-[Geist] text-lg">
        Detailed information about Color & Texture PPF will go here.
      </p>
    </div>
  );
}
