import SEO from '../../components/SEO';
export default function CeramicCoating() {
  return (
    <div className="min-h-screen pt-48 px-5 md:px-20 bg-[var(--color-background)]">
      <SEO title="Ceramic Coating | Detailing Demons" description="Learn more about our Ceramic Coating services." />
      <h1 className="text-4xl font-bold font-[Space_Mono] text-[var(--color-primary-container)] mb-8">
        Ceramic Coating
      </h1>
      <p className="text-[var(--color-on-surface-variant)] font-[Geist] text-lg">
        Detailed information about Ceramic Coating will go here.
      </p>
    </div>
  );
}
